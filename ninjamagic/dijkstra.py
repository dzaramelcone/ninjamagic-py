...
class DijkstraMap:
    ...
    def get_direction_toward(self, y: int, x: int) -> Compass | None:
    ...
    def get_direction_away(self, y: int, x: int) -> Compass | None:
        """Direction to highest cost neighbor (flee). Returns None if unreachable."""
        current_cost = self.get_cost(y, x)
        if current_cost == INF:
            return None
        ...
        for dy, dx, _ in _NEIGHBORS:
            ny, nx = y + dy, x + dx
            neighbor_cost = self.get_cost(ny, nx)
            # Flee to higher cost, but not to infinity (blocked)
            if neighbor_cost > best_cost and neighbor_cost != INF:
                best_cost = neighbor_cost
                best_dir = _VECTOR_TO_COMPASS.get((dy, dx))
