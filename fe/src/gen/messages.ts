...
class Health$Type extends MessageType<Health> {
    constructor() {
        super("ninjamagic.Health", [
            ...
            { no: 3, name: "stress_pct", kind: "scalar", T: 2 /*ScalarType.FLOAT*/ }
        ]);
    }
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* float stress_pct */ 3:
                    message.stressPct = reader.float();
                    break;
                default:
                    let u = options.readUnknownField;
                    if (u === "throw")
    ...
    internalBinaryWrite(message: Health, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        /* float stress_pct = 3; */
        if (message.stressPct !== 0)
            writer.tag(3, WireType.Bit32).float(message.stressPct);
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
