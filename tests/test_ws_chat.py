...
import pytest
from conftest import FakeUserSetup
...
@pytest.mark.asyncio
async def test_solo_client(golden_ws, client_factory, alice_setup) -> None:
    alice = await client_factory(alice_setup, discard_init=False)

    async with asyncio.timeout(1):
        golden_ws("alice", await alice.recv())
...
@pytest.mark.asyncio
async def test_moves(golden_ws, client_factory):
...
@pytest.mark.asyncio
async def test_combat_and_exp(golden_ws, client_factory):
    alice = await client_factory(
        FakeUserSetup(
            subj="alice",
            email="alice@x.test",
